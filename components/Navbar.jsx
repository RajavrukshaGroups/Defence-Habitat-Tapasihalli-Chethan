      ...
      >
        <div className="container mx-auto px-4 flex justify-between items-center transition-all duration-500">
          ...
          <div className="flex-3 flex flex-col items-center justify-center px-2 transition-all duration-500 ">
            <h1 className={`text-navy-800 font-condensed font-bold uppercase tracking-tighter leading-none text-center scale-y-110 transition-all duration-500 ${isScrolled ? 'text-xl lg:text-2xl' : 'text-2xl lg:text-2xl'}`}>
              DEFENCE HABITAT HOUSING CO-OPERATIVE SOCIETY LTD.
            </h1>
            <h2 className={`text-navy-800 font-bold mt-1 text-center font-serif leading-tight transition-all duration-500 ${isScrolled ? 'text-xs lg:text-sm' : 'text-sm lg:text-sm'}`}>
              ಡಿಫೆನ್ಸ್ ಹ್ಯಾಬಿಟಾಟ್ ಹೌಸಿಂಗ್ ಕೋ-ಆಪರೇಟಿವ್ ಸೊಸೈಟಿ ಲಿ.
            </h2>
            <div className={`text-center transition-all duration-500 ${isScrolled ? 'mt-0.5' : 'mt-1'}`}>
               <span className={`text-[#dc2626] font-sans font-bold uppercase tracking-tight transition-all duration-500 ${isScrolled ? 'text-sm lg:text-base' : 'text-base lg:text-base'}`}>
                 Reg. No.:- <span className="text-[#dc2626]">HSG-3/64/HHS/53744</span>
               </span>
            </div>
          </div>
          ...
          <div className="flex-1 flex justify-end items-center gap-3 ">
             <div className={`bg-[#e91e63] rounded-full text-white shadow-md transform hover:scale-110 transition-all duration-500 ${isScrolled ? 'p-1.5' : 'p-2 ml-16'}`}>
